...
for i in range(100):
    ...
    for j in range(len(ra)):
        ra1.append(np.random.normal(ra[j], np.abs(err[j])))
        dec1.append(np.random.normal(dec[j], err[j]))
...
for i in range(len(result[0])):
    ...
    corr_temp = []   
bins = np.linspace(0, 180, 100)
corr, dcorr, bootstraps = bootstrap_two_point_angular(ra, dec, bins, method = 'landy-szalay', Nbootstraps = 100, random_state = None)
...
# plt.title("SWIFT LGRB ABSOLUTE SUM TEST")
# plt.xlabel("bins")
